...
5. **참조 그래프 동기화** — FE→BE 호출이나 BE→BE 의존을 추가/삭제했으면(요소 링크·README `depends` 변경) 같은 작업에서 `docs/HOME.md`의 `## 참조 그래프` 엣지도 갱신한다. HOME이 의존의 단일 소스라 어긋나면 안 된다. (규약 [§분할 축](${DDD_ROOT}/docs/conventions.md))
