---
name: ticket
description: 정제(distill)와 아키텍처가 끝난 도메인을 콜드 스타트로 구현 가능한 자기완결 티켓(tickets/<fe|be>/NNNN-<slug>.md)으로 그릴하고 의존 순서 플로우를 그리는 스킬(기획 전단 종료). 책임 도메인 분류 → 코드 골격(code-map)으로 기존 클래스·패턴 파악 → 구조 결정(기존 재사용 우선·최소 추가, ARCHITECTURE 패턴 기본값 계승 vs 대안) → 엣지케이스 심층 분석. 산출 티켓은 scripts/implement.ps1 -Side <fe|be>로 구현 엔진에 넘겨 새 브랜치에서 "구현만" 시킨다. 티켓만 쓴다(코드·엔진 호출 안 함). "티켓 만들어줘", "이거 티켓화", "티켓 플로우 그려", "ticket" 류에 사용.
---
...
3. 후보 도메인 폴더 `docs/be/<name>/`(데이터.md·기능_*.md) 또는 `docs/fe/<name>/`(플로우.md·요소/*.md). `일지.md`는 **여태까지의 내역 요약이 담겨 있으니 읽어** 그간의 결정·변경 맥락을 잡는다.
4. 만질 코드 **골조** — 해당 도메인 소스가 있으면 통독하지 말고 `code-map.mjs`로 **시그니처 골격만**(이름만 긁어) 뽑아 이미 있는 클래스·함수·메서드·패턴을 파악한다. **전체 구조**는 §1.2 HOME 참조 그래프가, **작업부 골조**는 이 골격이 준다. 특정 본문이 필요할 때만 그 파일을 펼쳐 읽는다.
   ```
   node ${DDD_ROOT}/scripts/code-map.mjs --dir <도메인 README frontmatter의 code:, 기본 src/<slug>>
   ```
...
## 3. 구조 결정 — 재사용 우선 · 패턴 타협
**먼저 재사용(최소 추가).** 구조를 새로 짜기 전에 §1.4 코드 골격에 요청의 솔루션을 얹어본다 — 이미 있는 클래스·함수·패턴으로 풀리면 **그것을 확장·재사용**하고, 티켓엔 재사용할 실제 심볼(파일·클래스·메서드)을 지목한다. 새 클래스·추상·의존은 기존 능력으로 **정말 안 될 때만** 추가하고 *왜 기존으론 안 되는지* 한 줄로 못 박는다. 요청이 닿는 **정확한 지점만** 손대게 범위를 좁힌다(겸사겸사 리팩터·광범위 재작성 금지).

그 위에서 **패턴 타협**: ARCHITECTURE §4 도메인 기본값(TS/DM)에서 출발. 기능 성격이 기본값과 맞으면 그대로 + *왜* 한 줄. 어긋나면 트레이드오프 제안(A 기존 계승 / B 대안), 한 번에 한 갈림길, skeptic 기본. 규약 이탈이면 티켓에 근거 + 도메인 `일지` 한 줄. 코드로 답할 수 있으면 골격에서 짚고 필요한 본문만 읽는다.
...
- **재사용·최소 변경 지목** — §3에서 정한 **재사용할 기존 심볼**(파일·클래스·함수)과 **손댈 정확한 지점**을 적고, 새로 만드는 건 그 항목만 *왜 신규인지*와 함께 분리해 적는다.
- **범위 경계(하지 말 것)** 필수 — 기존으로 되는데 새 클래스·추상 만들기, 지목 밖 파일 수정, 겸사겸사 리팩터를 명시적으로 넣는다.
